...
emoji: '🦹‍♀️'
slug: 'vidar-infostealer-analysis'
publishDate: '2022-12-31 06:00:00'
excerpt: 'While reviewing samples submitted to Any.Run, I came across a recently-compiled binary that appeared to inject into a target process before performing some suspicious HTTP requests. After further analysis, this binary was found to be consistent with the VIDAR infostealer.'
categories: ['Malware Analysis', 'Cybercrime']
---

# Introduction

While reviewing samples submitted to Any.Run, I came across a [binary](https://app.any.run/tasks/8449e90f-586d-44e6-a277-f2be9aa0a2ca/) that appeared to inject into a target process before performing some suspicious HTTP requests. After further analysis, this binary was found to be consistent with the VIDAR infostealer.
This article aims to explain exactly how the infostealer works, the loader chain, what it attempts to steal, and how it exfiltrates data stolen from the host in a short time without leaving a trace.
...
![malware diagram](/images/vidar-infostealer-analysis/01-infection-diagram.png)

## About VIDAR

Although this is not unique to the VIDAR malware family, this infostealer performs a smash-and-grab approach to harvesting data by harvesting as much data from the host and exfiltrating as quickly as possible. Public reporting shows that delivery mechanisms for the VIDAR infostealer include [fake software installers](https://www.secureblink.com/cyber-security-news/tutorial-videos-on-you-tube-help-spread-vidar-and-raccoon-malware), [Windows 11 installers](https://www.zscaler.com/blogs/security-research/vidar-distributed-through-backdoored-windows-11-downloads-and-abusing) and even [malicious Microsoft help files](https://www.trustwave.com/en-us/resources/blogs/spiderlabs-blog/vidar-malware-launcher-concealed-in-help-file/) delivered via phishing.

Since the service provided to the customers of VIDAR exists only to facilitate payload configuration, generation and C2, it is left to the customer to get the malware on to systems; whether themselves or via a third-party malware distribution service. An example of such a malware distribution service is a group Microsoft tracks as [DEV-0569](https://www.microsoft.com/en-us/security/blog/2022/11/17/dev-0569-finds-new-ways-to-deliver-royal-ransomware-various-payloads/), who leverage techniques such as malvertising, phishing etc. to distribute BATLOADER; a separate malware designed to deliver additional payloads, including VIDAR, ROYAL ransomware and COBALT STRIKE.
...
![loader decryption routine](/images/vidar-infostealer-analysis/02-loader-decryption-routine.png)
...
![ntcreateuserprocess arguments](/images/vidar-infostealer-analysis/03-ntcreateuserprocess.png)

`NtCreateUserProcess` is an undocumented function, however more information about this function can be found [here](https://captmeelo.com/redteam/maldev/2022/05/10/ntcreateuserprocess.html), and has been well documented by security researchers. For the purposes of this loader, two important arguments are passed:
...
![virtualalloc arguments](/images/vidar-infostealer-analysis/04-virtualalloc-arguments.png)

**VirtualAllocEx Arguments**
![virtualallocex arguments](/images/vidar-infostealer-analysis/05-virtualallocex-arguments.png)
...
![writeprocessmemory arguments](/images/vidar-infostealer-analysis/06-writeprocessmemory-arguments.png)
...
![memory map of buffer space](/images/vidar-infostealer-analysis/07-memorymap.png)
...
![realigned section table](/images/vidar-infostealer-analysis/09-realigned-sections.png)

Now the executable has been correctly aligned, the module and function imports are now readable.

![readable imports](/images/vidar-infostealer-analysis/10-readable-imports.png)
...
The VIDAR malware stores some of its strings, and module imports in a base64-encoded, RC4 encrypted format. By combining the different functions in the binary, along with the decrypted and decoded strings, we arrive at a full list of browser extensions targeted by the malware.
These are mainly cryptocurrency wallets, but also include two factor authentication (2FA) extensions, and other password managers.

Before doing anything else, VIDAR first calls a built-in routine to base64-decode and RC4 decrypt each string before writing it into a memory region.
A pointer to each string and module import can then be referenced by the malware to use them.
...
![steam profile](/images/vidar-infostealer-analysis/11-steam-profile.png)
![telegram profile](/images/vidar-infostealer-analysis/12-telegram-profile.png)
...
![ida update.zip](/images/vidar-infostealer-analysis/13-update-zip.png)
...
![IDA post](/images/vidar-infostealer-analysis/14-ida-post.png)
...
![ida cmd](/images/vidar-infostealer-analysis/15-ida-cmd.png)